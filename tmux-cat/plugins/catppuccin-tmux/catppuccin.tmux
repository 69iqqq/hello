...
build_window_format() {
  ...
  if [ "$fill" = "number" ]; then
    ...
    local show_text="#[fg=#cdd6f4,bg=default]$text"
  ...
  if [ "$window_number_position" = "right" ]; then
    final_window_format="$show_text$show_middle_separator$show_number$show_right_separator"
  fi

  if [ "$window_number_position" = "left" ]; then
    final_window_format="$show_number$show_middle_separator$show_text$show_right_separator"
  fi
...
build_status_module() {
  ...
  if [ "$status_fill" = "icon" ]; then
    ...
    local show_text="#[fg=#cdd6f4,bg=$thm_gray] $text"
